<div class="navbar-custom">
    ...
    <div class="logo-box">
        <a href="{{ route('admin-dashboard') }}" class="logo text-center">
            <span class="logo-lg">
                <img @if(!empty($settings->logo)) src="{{
                asset($settings->logo) }}"
                @else
                src="{{asset('web/admin/assets/images/logo-light.png')}}"
                @endif alt="" height="18">
            </span>
            <span class="logo-sm">
                <img @if(!empty($settings->fav_icon)) src="{{
                asset($settings->fav_icon) }}"
                @else
                src="{{asset('web/admin/assets/images/logo-sm.png')}}"
                @endif alt="" height="24">
            </span>
        </a>
    </div>
